...
class KR:
    ...
    def solve_olimp(self, ID, start_index, scopeNodes, n, algorithm):
        ...
        if algorithm == "A*":
            result = self.a_star(start_index, scopeNodes, n)
            if len(result) == 1:
                print(f"Scope node reached: {result[0]}")
            else:
                print("Open set with heuristic values:", sorted(result))
        elif algorithm == "IDA*":
            path = kr.IDA_star(start_index, scopeNodes, n)
            if path:
...
if __name__ == "__main__":
    ...
    kr = KR(None)
    kr.solve_olimp(ID, start_index, scopeNodes, n, algorithm)